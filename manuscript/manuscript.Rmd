---
title: "Covid-19 infection and attributable mortality in residents of UK care homes: Cohort study using electronic records (March-June 2020)"
...
incidence_CT_residents <- table_nums(
  ...
  caption =  paste0("Incidence proportions and rates of SARS-CoV-2 infections in residents according to FSHCG manager counts (", format(date_min, "%e %b %Y"), "-", format(date_max, "%e %b %Y"), ")"))
supp_epi_CT_oubreak_only <- table_nums(
  name = "incidence_CT_staff",
  caption =  paste0("Incidence proportions and rates of SARS-CoV-2 infections among staff according to FSHCG manager counts (", format(date_min, "%e %b %Y"), "-", format(date_max, "%e %b %Y"), ")"))
...
tab_CFR_age_sex <- table_nums(
  ...
  caption =  paste0("All-cause case-fatality rates and relative risks by age and sex among private residents (n=", 
...
tab_CoxPH_death <- table_nums(
  ...
  formatbm(dplyr::n_distinct(residents_episodes_short$resident_id)),
  ", ", format(date_min_cox_mortality, "%e %b %Y"), "-", format(date_max, "%e %b %Y"), ")"))
...
`r desc_residents$CT_zero_case`/`r desc_residents$total_homes` homes (`r tbrounding( desc_residents$CT_zero_case/desc_residents$total_homes*100)`%) had neither confirmed infections nor Covid-related deaths.

Among `r formatbm(desc_residents$DAT_total_residents)` private residents, there were `r formatbm(desc_residents$DAT_total_deaths)` all-cause deaths (`r tbrounding(desc_residents$DAT_p_death)`% [`r tbrounding(desc_residents$DAT_p_death_lower)`%; `r tbrounding(desc_residents$DAT_p_death_upper)`%]), `r desc_residents$DAT_CFR$y` of which occurred in residents with confirmed infection, equivalent to an all-cause case-fatality rate of `r tbrounding(desc_residents$DAT_CFR_rate)`% [`r tbrounding(desc_residents$DAT_CFR_rate_lower)`%; `r tbrounding(desc_residents$DAT_CFR_rate_upper)`%]. `r tbrounding(((desc_residents$DAT_CFR_RR - 1)/desc_residents$DAT_CFR_RR)*100)`% [`r tbrounding(((desc_residents$DAT_CFR_RR_lower - 1)/desc_residents$DAT_CFR_RR_lower)*100)`%; `r tbrounding(((desc_residents$DAT_CFR_RR_upper - 1)/desc_residents$DAT_CFR_RR_upper)*100)`%] of these deaths were attributable to SARS-CoV-2. Symptomatic and asymptomatic infection were independent risk factors for mortality. All-cause mortality for uninfected residents was 93% higher in care homes with outbreaks compared to those without outbreaks (adjusted HR = 1·9 [1·7; 2·2]).
...
Care home residents are particularly vulnerable to Covid-19 infection due to their advanced age and high prevalence of comorbidity,[@Zhou2020] and this is compounded by frequent exposure to infection through close contact with infected staff members, other residents and contaminated surfaces in the care home setting. In the UK, an estimated 420,000 residents lived in approximately 11,000 homes for the elderly.[@cqcdata2020] 

At the peak of the pandemic the number of deaths in care home residents was three-fold higher than the equivalent period in in 2019.[@onsDR2020]  Data from the Office for National Statistics suggest that two-thirds of the 29,393 excess deaths recorded in care home residents between 28th December 2019 and 12th June 2020 (calculated relative to equivalent period in the previous year) involved Covid-19.[@onsDR2020] However, estimating the proportion of care home deaths that can be directly attributed to Covid-19 based on death certifications is challenging because many care home residents with suspected infection were not tested, particularly at the start of the pandemic. Understanding the ratio of direct to indirect deaths in care home residents will be critical to inform public health measures to mitigate the impact of the anticipated second wave of the pandemic, since lockdown measures that prevent infection may have a negative impact on residents by disrupting usual care.

Disentangling direct and indirect deaths from Covid-19 requires accurate information on the incidence and prevalence of infection among staff and residents linked to individual-level clinical outcomes. There is no syndromic surveillance for infection in care homes in England, and universal testing for SARS-CoV-2 using reverse transcriptase polymerase chain reaction (RT-PCR) was not established in care homes in England until 11th May 2020, when the government launched the whole care home testing programme.[@dhscTesting2020] Prior to this, testing was only available for care home residents or staff who were admitted to hospital, or as part of Public Health England's (PHE) outbreak investigations with a maximum of five tests per care home. Consequently national estimates of incidence and prevalence based on test results obtained prior to 11th May will substantially underestimate the burden of infection in care home residents and staff.

Outbreak investigations provide estimates of the proportion of residents and staff who become infected during an outbreak and case-fatality rates,[@McMichael2020; @Dora2020] but they provide limited insight into the relationship between infection and mortality across the entire pandemic period, and cannot be generalised to care homes that have not reported any outbreaks. A living systematic review[@Salcher-Konrad2020] of outbreak investigations found incidence proportions ranging between 0% and 72% among residents and between 2% and 64% among staff of long-term care facilities. Mortality rates in residents with confirmed Covid-19 infection ranged from 0 to 10%, with case fatality rates between 0 and 34%, however this review did not include estimates of indirect mortality.
...
## Aggregate outbreak surveillance counts (`r format(date_min_tallies, "%e %b %Y")`--`r format(date_max_tallies, "%e %b %Y")`)
...
## Individual-level residents records (`r format(date_min, "%e %b %Y")`--`r format(date_max, "%e %b %Y")`)
...
Individual-level data (including dates of care home entry and exit) were available for private residents only. By assuming infection underreporting to affect residents conditionally at random, it was possible to use the data to study factors associated with the rate of infection and mortality. A Cox proportional hazards model was used to test the effect of both individual-level risk factors (sex, age, care type) and organisation-level risk factors (region, care home size, staff-to-bed ratio, occupant-to-bedroom ratio).

). We also compared care ratesDates of infectiondischarge were used alongside the death indicator to estimate the case-fatality rate in home and hospital, as well as compare all-cause mortality in care homes withbased on the presence of symptoms and without outbreaks, defined as at least one /or confirmed case or Covid-19 related death during the study period. infection. A time-dependent Cox proportional hazards model analysed the effect of the same factors on all-cause mortality based on the time-dependent infection classification (susceptible; symptomatic not confirmed; asymptomatic confirmed; symptomatic confirmed). We also compared rates of infection and mortality in care homes with and without outbreaks, defined as at least one confirmed infection or Covid-19 related death during the study period. A time-dependent Cox proportional hazards model analysed the effect of the same factors on all-cause mortality based on the time-dependent infection classification (uninfected; symptomatic not confirmed; asymptomatic confirmed; symptomatic confirmed) as well as the time-dependent home outbreak status (no Covid-related death or confirmed infection; at least one Covid-related death or confirmed infection). The associated hazard ratios enable us to compare the mortality in each group as compared to the reference category (uninfected in non-outbreak care homes). Interpretation is facilitated by computing the associated attributable risk $ AR = (HR - 1)/HR$, which  

https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5259851/pdf/12874_2016_Article_285.pdf
...
Bedroom occupancy and staffing ratios were estimated on the basis of baseline occupancy averaged before the first cases (between 1 January and 1 March 2020), so as to rule out reverse causality. Dates of discharge were used alongside the death indicator to estimate the case-fatality rate in home and hospital, as well as compare all-cause mortality based on the presence of symptoms and/or confirmed infection. Confirmed infection data in both sources can only be treated as underestimates of the true incidence, due to the gradual increase in nasopharyngeal swab (PCR) testing across the study period following the introduction of whole care home testing on 11 May 2020.[@dhscTesting2020]. By 22 June 2020, 90% of FSHCG care homes had participated in the whole home testing programme.
...
Between `r format(date_min_tallies, "%e %B %Y")` and `r format(date_max_tallies, "%e %B %Y")`, `r filter(new_cases, new_symptomatic_home>0 | new_symptomatic_hospital>0) %>% summarise(n=n_distinct(home_code)) %>% .$n`/`r desc_residents$total_homes` care homes reported `r formatbm(sum(new_cases$new_symptomatic_home, na.rm = T))` residents who developed symptoms in the care home and `r formatbm(sum(new_cases$new_symptomatic_hospital, na.rm = T))` residents who developed symptoms following readmission from hospital, contributing to an overall incidence proportion of `r tbrounding(desc_residents$CT_p_symptomatic)`% [`r tbrounding(desc_residents$CT_p_symptomatic_lower)`%; `r tbrounding(desc_residents$CT_p_symptomatic_upper)`%] or an incidence rate of `r tbrounding(desc_residents$CT_IR_symptomatic)` per 100,000 [`r tbrounding(desc_residents$CT_IR_symptomatic_lower)`; `r tbrounding(desc_residents$CT_IR_symptomatic_upper)`] (`r table_nums("incidence_CT_residents", display = "cite")`). Infection was confirmed in `r desc_residents$CT_total_confirmed` residents, of whom `r formatbm(sum(new_cases$new_confirmed_hospital, na.rm = T))` were diagnosed in hospital. The confirmed infection incidence proportion was `r tbrounding(desc_residents$CT_p_confirmed)`% [`r tbrounding(desc_residents$CT_p_confirmed_lower)`%; `r tbrounding(desc_residents$CT_p_confirmed_upper)`%], with an incidence rate of `r tbrounding(desc_residents$CT_IR_confirmed)` per 100,000 [`r tbrounding(desc_residents$CT_IR_confirmed_lower)`; `r tbrounding(desc_residents$CT_IR_confirmed_upper)`]. 

Kaplan-Meier estimates of the temporal trend in symptomatic and confirmed cases are shown on `r fig_nums("fig_KM_CT_plots", display = "cite")`. The sharp increase in confirmed infections between 30 April and 2 May (67 new cases) can largely be attributed to the piloting of whole-home testing in one care home, leading to 42 cases being confirmed. 


In England only, `r england_results$FSHC_confirmed` infections were confirmed of a total `r formatbm(england_results$FSHC_resident_days)` residents-days between `r format((england_results$date_min), "%e %B %Y")` and `r format(england_results$date_max, "%e %B %Y")`. In comparison, the survey of English community households[@cisbulletin2020] found a total of `r england_results$cis_confirmed` confirmed cases out of `r formatbm(england_results$cis_person_days)` person-days during this period. This implies a confirmed infection rate ratio of `r tbrounding(england_results$results$measure[2,"estimate"])` [`r tbrounding(england_results$results$measure[2,"lower"]) `; `r tbrounding(england_results$results$measure[2,"upper"])`].
...
```{r fig_KM_CT_plots, fig.height = 4.5, fig.width = 14, message=FALSE}
...
Symptoms of infection were reported by `r formatbm(desc_staff$CT_symptomatic_total)`/`r formatbm(desc_staff$total_staff)`  staff (`r tbrounding(desc_staff$CT_symptomatic_p)`% [`r tbrounding(desc_staff$CT_symptomatic_p_lower)`%; `r tbrounding(desc_staff$CT_symptomatic_p_upper)`%]) during the study period, and `r formatbm(desc_staff$CT_confirmed_total)` staff (`r tbrounding(desc_staff$CT_confirmed_p)`% [`r tbrounding(desc_staff$CT_confirmed_p_lower)`%; `r tbrounding(desc_staff$CT_confirmed_p_upper)`%]) had a confirmed infection (`r table_nums("incidence_CT_staff", display="cite")`, `r fig_nums("fig_KM_CT_plots", display = "cite")`).
...
Incidence estimates for `r formatbm(desc_residents$DAT_total_residents)` private residents derived from Datix incident records are reported in supplementary material 2. Lower estimates were obtained which is likely due to reflect incomplete recording of cases in Datix, and the inability to link all incident records to an individual.


## Mortality in residents

`r desc_residents$CT_total_cdeath` Covid-related resident deaths were reported across all care homes, equivalent to a crude incidence of `r tbrounding(desc_residents$CT_p_cdeath)`% [`r tbrounding(desc_residents$CT_p_cdeath_lower)`; `r tbrounding(desc_residents$CT_p_cdeath_upper)`] or `r  tbrounding(desc_residents$CT_IR_cdeath)` [`r  tbrounding(desc_residents$CT_IR_cdeath_lower)`; `r  tbrounding(desc_residents$CT_IR_cdeath_upper)`] per 100,000 resident-days. `r tbrounding(sum(new_cases$new_deaths_hospital, na.rm = T)/desc_residents$CT_total_cdeath*100)`% of all Covid-19-related deaths reported by outbreak surveillance counts took place in hospital.

Individual-level data were available for a subset of `r formatbm(desc_residents$DAT_total_residents)` (`r tbrounding(desc_residents$DAT_total_residents/desc_residents$CT_total_residents*100)`%) private residents, the vast majority (85·4%) of whom were permanent residents.  `r tbrounding(mean(residents$home_type=="Nursing")*100)`% of private residents were in nursing homes, and 39·2% received dementia care (`r table_nums("tab_resident_descriptives", display = "cite")`). The crude incidence of all-cause death in private residents was `r tbrounding(desc_residents$DAT_p_death)`% [`r tbrounding(desc_residents$DAT_p_death_lower)`%; `r tbrounding(desc_residents$DAT_p_death_upper)`%]. All cause mortality in residents with confirmed infection was `r tbrounding(desc_residents$DAT_CFR_rate)`% [`r tbrounding(desc_residents$DAT_CFR_rate_lower)`%; `r tbrounding(desc_residents$DAT_CFR_rate_upper)`%]. This is to be contrasted with a mortality of `r tbrounding(desc_residents$DAT_BFR_rate)`% [`r tbrounding(desc_residents$DAT_BFR_rate_lower)`%; `r tbrounding(desc_residents$DAT_BFR_rate_upper)`%] in private residents that did not test positive for Covid-19 (RR = `r tbrounding(desc_residents$DAT_CFR_RR)` [`r tbrounding(desc_residents$DAT_CFR_RR_lower) `; `r tbrounding(desc_residents$DAT_CFR_RR_upper)`]). The excess death fraction attributable to confirmed infection is thus `r tbrounding(((desc_residents$DAT_CFR_RR - 1)/desc_residents$DAT_CFR_RR)*100)` [`r tbrounding(((desc_residents$DAT_CFR_RR_lower - 1)/desc_residents$DAT_CFR_RR_lower)*100)`; `r tbrounding(((desc_residents$DAT_CFR_RR_upper - 1)/desc_residents$DAT_CFR_RR_upper)*100)`].
...
arsenal::tableby(
  outbreak  ~ Sex + Age  +  `Resident type` + `Admission type`  + `Home type` +  
    `Infection status by 14 June` + `Status as of 14 June` + `Region/nation`, 
...
To investigate whether increased mortality was only seen in individuals who were infected with Covid-19, we compared mortality in residents with no record of Covid-19 from care homes with outbreaks and those without. 
`r formatbm(desc_residents$DAT_BFR_outbreak_homes$y)`/`r formatbm(desc_residents$DAT_BFR_outbreak_homes$n)` died in outbreak homes, compared with `r formatbm(desc_residents$DAT_BFR_other_homes$y)`/`r formatbm(desc_residents$DAT_BFR_other_homes$n)` in non-outbreak homes. The risk of all-cause death was therefore twice as large in outbreak homes for uninfected residents (RR = `r tbrounding(desc_residents$DAT_BFR_outbreak_homes$RR)` [`r tbrounding(exp(log(desc_residents$DAT_BFR_outbreak_homes$RR) - 1.96 * desc_residents$DAT_BFR_outbreak_homes$RR_SE))`; `r tbrounding(exp(log(desc_residents$DAT_BFR_outbreak_homes$RR) + 1.96 * desc_residents$DAT_BFR_outbreak_homes$RR_SE))`]). This implies that `r tbrounding((desc_residents$DAT_BFR_outbreak_homes$RR-1)/desc_residents$DAT_BFR_outbreak_homes$RR*100)`% [`r tbrounding((desc_residents$DAT_BFR_outbreak_homes$RR_lower-1)/desc_residents$DAT_BFR_outbreak_homes$RR_lower*100)`%; `r tbrounding((desc_residents$DAT_BFR_outbreak_homes$RR_upper-1)/desc_residents$DAT_BFR_outbreak_homes$RR_upper*100)`%] of deaths in residents who did not test positive for infection in outbreak homes were in some way attributable to Covid-19.
...
The time-dependent Cox proportional hazard models in `r table_nums("tab_CoxPH_death", display = "cite")` examine the relationship between infection status and mortality. After controlling for other risk factors, increased mortality was associated with older age, male gender (adjusted HR = 1·5 [1·3; 1·6]), and receiving nursing rather than residential care (adjusted HR = 1·5 [1·3; 1.7]). All-cause death was strongly associated with asymptomatic confirmed infection (adjusted HR = 3·3 [1·9; 5·6]) and symptomatic confirmed infection (adjusted HR = 12 [10; 15]), compared to residents with no evidence of Covid-19 infection.

We estimated excess mortality in outbreak and non-outbreak homes, taking individuals with no evidence of infection in non-outbreak homes as the reference group. Risk of all-cause mortality was almost two-fold higher in residents with no evidence of infection in outbreak versus non-outbreak homes (adjusted HR = 1·9 [1·7; 2·2]) in outbreak homes than in other homes. Risk of death was also higher in individuals with symptoms but unconfirmed infection in outbreak homes compared to non-outbreak homes (adjusted HR = 4·4 [3·0; 6·2] versus 9·3 [7·5; 12]).
...
After adjusting for other variables, an increase by 10 percentage points in staffing was associated with a 3·1% reduction [0·6%; 5·7%] in mortality after adjustment for other  variables.
...
# residents_daily <- residents_daily %>% 
#   group_by(date) %>% 
#   summarise(n = sum(rday),
#             d = sum(death),
#             covid_status_outbreak_digit0 = 1 - 
#               mean(covid_status_outbreak_digit1) -
#               mean(covid_status_outbreak_digit2) -
#               mean(covid_status_outbreak_digit3) -
#               mean(covid_status_outbreak_digit4) -
#               mean(covid_status_outbreak_digit5),
#             covid_status_outbreak_digit1 = mean(covid_status_outbreak_digit1),
#             covid_status_outbreak_digit2 = mean(covid_status_outbreak_digit2),
#             covid_status_outbreak_digit3 = mean(covid_status_outbreak_digit3),
#             covid_status_outbreak_digit4 = mean(covid_status_outbreak_digit4),
#             covid_status_outbreak_digit5 = mean(covid_status_outbreak_digit5)) %>% 
#   mutate(date_int = as.integer(date - date_min))
...
#                       gender+ ageg + home_type + covid_status_outbreak +
#                       staff_to_bed_ratio + bed_tot_category + occ_bedroom_ratio_g6 +
#                       cluster(resident_id), data = residents_episodes_short, ties = "breslow")
# summary(cph_death2)
# cox.zph(cph_death2)
...
CPH_death <-  finalfit(residents_episodes_short, dependent, explanatory, 
...
KM_mortality_by_covid_status <- survfit(
  Surv(time = date1, time2 = date2, event = death_status) ~ covid_status + outbreak, 
  data = residents_episodes_short)
KM_mortality_plot <- ggsurvplot_facet(KM_mortality_by_covid_status,
                 ...
                 scales = "free", ggtheme = theme_bw(), xlim = c(32, 104),
...
cph_death2 <- coxph(Surv(date1, date2, death_status) ~
                      gender+ ageg + home_type + covid_status_outbreak +
                      staff_to_bed_ratio + bed_tot_category + occ_bedroom_ratio_g6 +
                      cluster(resident_id), data = residents_episodes_short)
HRs_covid <- tibble::enframe(cph_death2$coefficients[grep("covid_status_outbreak", 
                                                          names(cph_death2$coefficients))]) %>% 
  transmute(covid_status_outbreak = gsub("covid_status_outbreak", "", name),
            HR = exp(value))
HRs_CI_covid <- as_tibble(exp(confint(cph_death2))[grep("covid_status_outbreak", 
                                                          names(cph_death2$coefficients)),]) %>% 
  rename(HR_lower =  `2·5 %`,
         HR_upper = `97·5 %`)
HRs_CI_covid$covid_status_outbreak <- gsub(
  "covid_status_outbreak", "",
  grep("covid_status_outbreak", names(cph_death2$coefficients), value = T)
) 
HRs_covid <- merge(HRs_covid, HRs_CI_covid)
  
attributable_deaths <- residents_episodes_short %>% 
  filter(death_status == 1) %>% 
  group_by(covid_status_outbreak) %>% 
  summarise(Deaths = n()) %>% 
    left_join(HRs_covid) %>% 
    ...
    mutate(AH = (HR - 1)/HR,
           AH_lower = (HR_lower - 1)/HR_lower,
           AH_upper = (HR_upper - 1)/HR_upper) %>% 
    mutate(`Attributable deaths` = AH * Deaths) %>% 
    mutate(`AD (%)` = tbrounding(`Attributable deaths`/ sum(Deaths)*100)) %>% 
  transmute(`Infection/outbreak status` = covid_status_outbreak,
            Deaths,
            HR = paste0(tbrounding(HR,2), " [",
                        tbrounding(HR_lower,2),"; ",
                        tbrounding(HR_upper,2), "]"),
            `Attibutable hazard (%)`= paste0(tbrounding(AH*100,1), " [",
                        tbrounding(AH_lower*100,1),"; ",
                        tbrounding(AH_upper*100,1), "]"),
           `Attributable deaths` = round(`Attributable deaths`),
           `AD (%)`)
attributable_deaths <- bind_rows(
  attributable_deaths,
  tibble(
    `Infection/outbreak status` = "TOTAL",
            Deaths = (sum(attributable_deaths$Deaths)),
            HR = "",
            `Attibutable hazard (%)`= "",
           `Attributable deaths` = sum(attributable_deaths$`Attributable deaths`),
           `AD (%)` = tbrounding(
             sum(attributable_deaths$`Attributable deaths`)/sum(attributable_deaths$Deaths)*100)
  )
)

kable(attributable_deaths, align =c("l", "r", "r", "r", "r", "r", "r"))
```
...
# residents_episodes$resident_id_int <- as.numeric(as.factor(residents_episodes$resident_id))
# cph_death2 <- coxph(Surv(date1, date2, death_status) ~
#                       gender+ ageg + home_type + covid_status_outbreak_digit1 +
#                       covid_status_outbreak_digit2 + covid_status_outbreak_digit3+
#                       covid_status_outbreak_digit4 +covid_status_outbreak_digit5 +
#                       staff_to_bed_ratio + bed_tot_category + occ_bedroom_ratio_g6 +
#                       cluster(resident_id_int), data = data.frame(residents_episodes), 
#                     ties = "breslow")
# 
# trucmuche <- survfit(Surv(date1, date2, death_status) ~
#                       gender+ ageg + home_type + covid_status_outbreak_digit1 +
#                       covid_status_outbreak_digit2 + covid_status_outbreak_digit3+
#                       covid_status_outbreak_digit4 +covid_status_outbreak_digit5 +
#                       staff_to_bed_ratio + bed_tot_category + occ_bedroom_ratio_g6,
#                      residents_episodes_short, id = "resident_id")
# 
# AH_fractions <- residents_daily %>% 
#   tidyr::pivot_longer(cols=starts_with("covid_status_outbreak_")) %>% 
#   dplyr::rename(prevalence = value) %>% 
#   left_join(tibble::enframe(cph_death2$coefficients[grep("covid_status_outbreak_digit", names(cph_death2$coefficients))])) %>% 
#   mutate(AH = (prevalence * tidyr::replace_na(exp(value) - 1, 0)) / 
#            (prevalence * tidyr::replace_na(exp(value) - 1, 0) +1))
# 
# 
# AH_coefs <- tibble::enframe(cph_death2$coefficients[grep("covid_status_outbreak_digit",
#                                                          names(cph_death2$coefficients))]) %>% 
#   rename(covid_status_outbreak_digit = name) %>% 
#   mutate(covid_status_outbreak_digit = gsub("covid_status_outbreak_digit", "", covid_status_outbreak_digit))
#                               
# AH_fractions <- residents_daily %>%
#   group_by(date, covid_status_outbreak_digit) %>%
#   summarise(n = sum(rday),
#             d = sum(death)) %>% 
#   left_join(AH_coefs)  %>% 
#   filter(date >= as.Date("2020-04-01")) %>% 
#   mutate(AH =  (tidyr::replace_na(exp(value), 1) -1 ) / tidyr::replace_na(exp(value), 1)) %>% 
#   group_by(covid_status_outbreak_digit) %>% 
#   summarise(AH = sum(AH*d),
#             d = sum(d),
#             n = sum(n))  
#             
# 
...
# 
# ggplot(AH_fractions)+
#   geom_line(aes(x = date, y = AH*d, color = name ))
# 
...
# AF_estimates <- list()
# for(i in 1:5) {
#   AF_estimates[[levels(residents_episodes$covid_status_outbreak)[i]]] <- 
#     AF::AFcoxph(object = cph_death2, data = data.frame(residents_episodes), 
#             exposure = paste0("covid_status_outbreak_digit", i), clusterid = "resident_id_int")
#   AF_estimates[[i]]$se <- sqrt(AF_estimates[[i]]$AF.var)
#   AF_estimates[[i]]$confidence.interval <- AF:::CI.AF(
#     AF = AF_estimates[[i]]$AF.est, 
#     Std.Error = AF_estimates[[i]]$se, 
#     confidence.level = 0.95, CI.transform = "log")
#   colnames(AF_estimates[[i]]$confidence.interval) <- c("Lower limit", "Upper limit")
#   AF_estimates[[i]]$covid_status_outbreak <- levels(
#     residents_episodes$covid_status_outbreak)[i+1]
#   AF_estimates[[i]] <- bind_cols(
#     as_tibble(
#       AF_estimates[[i]][c('times', 'covid_status_outbreak', 'AF.est', 'AF.var', 
#                          'S.est', 'S0.est', 'S.var', 'S0.var')]
#     ),
#     as_tibble(AF_estimates[[i]]$confidence.interval)
#   )
# }
# 
# 
# ggplot(bind_rows(AF_estimates[2:5]),
#        aes(x = times,  color =  covid_status_outbreak )) +
#   geom_ribbon(aes(ymin = `Lower limit`*100, ymax = `Upper limit`*100), 
#               alpha=0.05, linetype = 2) +
#   geom_line(aes(y=AF.est*100), size = 1) +
#   coord_cartesian(ylim=c(0, 30)) +
#   ggsci::scale_color_lancet() +
#   xlab("Excess mortality (%)") +
#   ggtitle("% excess mortality compared with uninfected in non-outbreak homes")
# 
# # 
# cph_death2 <- coxph(Surv(date1, date2, death_status) ~
#                         covid_status_outbreak_digit  +
#                       cluster(resident_id_int), data = residents_episodes, 
#                     ties = "breslow")
# cph_death2 <- flexsurv::flexsurvreg(Surv(date1, date2, death_status) ~
#                         covid_status_outbreak_digit  +
#                       cluster(resident_id), 
#                       data = residents_episodes_short,dist = "weibull")
# 
# truc <- survfit(cph_death2, tibble(date1 = 30, covid_status_outbreak_digit = factor(1:5, levels = 1:5) )) 
#   ggsurvplot(truc,
#              data = tibble(date1 = 30, covid_status_outbreak_digit = factor(5, levels = 1:5) ))
#     
```
