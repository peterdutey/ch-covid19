...
desc_residents_outbreak_only$outbreak_homes <- unique(
  filter(home_outbreak_sensitivity, !is.na(first_case_date) == 1)$home_code
)
...
To adjust for case under-ascertainment, we created a dummy variable indicating 'affected care homes', which we define throughout the manuscript as care homes that recorded $\geq$1 confirmed infection or COVID-19 related death during the period.
...
Time-dependent Cox proportional hazard models (`r table_nums("tab_CoxPH_death", display = "cite")`) examine the relationship between infection status (groups A–D) and mortality mortality (Figure S3). After controlling for other risk factors, increased mortality was independently associated with older age, male gender (adjusted HR = 1.5 [1.3; 1.6]), and receiving nursing care (adjusted HR = 1.4 [1.1; 1.6]).

We estimated excess mortality in outbreak and non-outbreak care homes, taking individuals with no evidence of infection (group A) in non-outbreak care homes as the reference group. Hazards of all-cause mortality were two-fold higher in Group A – no direct evidence of infection in outbreak versus non-outbreak care homes (adjusted HR = 2.1 [1.7; 2.5]). All-cause mortality was strongly associated with confirmed infection, whether asymptomatic (Group C: adjusted HR = 3.4 [2.1; 5.7]) or symptomatic (Group D: adjusted HR = 13 [10; 16]). In confirmed infections, mortality was significantly higher in individuals with a record of symptoms.
...
Model-based estimates indicate that 633/1,694 (37%) all-cause deaths were attributable to COVID-19 (Table S7). In care homes with outbreaks only, just 161/609 (26%) deaths attributa-ble to COVID-19 occurred in people with confirmed infection (Groups C and D, Table S7).

`r table_nums("tab_attributable_deaths")`

```{r attributable_deaths, warning=F, message=F, results = "asis"}
cph_death3 <- coxph(Surv(date1, date2+0.1, death_status) ~
...
In common with a Canadian cohort study,[@Brown2020] we found strong associations between infections and care home baseline occupancy. We also found staffing levels to be negatively associated with infection rates. These organisational factors, linked to chronic underfunding of the care sector, are likely to facilitate the implementation of infection control procedures[@dhICguideline2020] such as isolating or cohorting infected residents, staff training, and regular environmental deep cleaning. When staff care for fewer residents they also have reduced likelihood of spreading infection between residents. Higher staff to resident ratios may also decrease reliance on agency staff who often work across multiple settings, and indicate better resourced care homes. This association may also be confounded by other characteristics of homes with higher staffing levels, if those happen to be better resourced, for instance in relation to staff training, availability of personal protective equipment, building characteristics.
