...
occ_home <- merge(
  ...
  na.omit(select(filter(occupancy, home_code != "360"), 
...
During the study period, manager counted `r sum(ch_data_all$tally_confirmed)` confirmed infection while Datix recorded `r sum(ch_data_all$datix_confirmed)` such infections. The `r round(sum(ch_data_all$tally_confirmed)/sum(ch_data_all$datix_confirmed)*100-100 )`% excess could be due to a range of explanations:
