...
residents_episodes <- tline %>% 
  ...
  left_join(select(reference_homes, home_code, country, region_UK,
                   ...
                   beds, pct_occupancy, occ_bedroom_ratio, no_bedrooms)) %>% 
...
home_total_confirmed <- distinct(reference_homes, home_code, home_name_clickview, 
                                 ...
                                 bed_tot_category, beds, pct_occupancy, no_bedrooms, occ_bedroom_ratio,
...
# cph_conf <- coxph(Surv(time = start_int, time2 = stop_int +1, event = confirmed) ~ gender + ageg+home_type+user_type2+region_UK+ staff_to_bed_ratio+bed_tot_category + pct_occupancy+occ_bedroom_ratio, data = residents_overview)
#  summary(cph_conf)
# cox.zph(cph_conf)
...
#                       pct_occupancy + occ_bedroom_ratio + cluster(resident_id), data = residents_episodes)
# summary(cph_death2)
# cox.zph(cph_death2)
...
explanatory = c("gender", "ageg", "home_type", #"user_type2", 
                "staff_to_bed_ratio", "bed_tot_category", "pct_occupancy", # "occ_bedroom_ratio",
