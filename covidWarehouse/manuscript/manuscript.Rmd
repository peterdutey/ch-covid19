...
desc_overall <- list()
desc_overall$total_residents <- dplyr::n_distinct(residents_overview$resident_id)
desc_overall$total_staff <- reference_homes %>% filter(home_code %in% unique(residents_overview$home_code)) %>% .$staff_total %>% sum()
...
desc_overall$DAT_p_confirmed <- desc_overall$unique_residents_test_result_positive/desc_overall$total_residents
desc_overall$DAT_p_confirmed_ME <- 1.96 * sqrt(
  desc_overall$DAT_p_confirmed * (1-desc_overall$DAT_p_confirmed) / 
    desc_overall$total_residents
)

desc_overall$DAT_incidence_confirmed <- desc_overall$unique_residents_test_result_positive/sum(residents_overview$susceptible_confirmed_days)
desc_overall$DAT_incidence_confirmed_SE <- sqrt((1-desc_overall$DAT_incidence_confirmed)/desc_overall$unique_residents_test_result_positive)
desc_overall$DAT_incidence_confirmed_95low <- exp(
  log(desc_overall$DAT_incidence_confirmed - 1.96 * desc_overall$DAT_incidence_confirmed_SE)
)
desc_overall$DAT_incidence_confirmed_95up <- exp(
  log(desc_overall$DAT_incidence_confirmed + 1.96 * desc_overall$DAT_incidence_confirmed_SE)
)
...
england_results$n_positive = sum(inner_join(new_cases, filter(reference_homes, country == "England"))[c("new_confirmed_home", "new_confirmed_hospital")], na.rm = T)
england_results$n_negative = n_distinct(inner_join(residents, filter(reference_homes, country == "England"))$resident_id) - england_results$n_positive
...
**Findings:** Records were available for `r formatbm(n_distinct(residents_overview$resident_id))` residents present in `r dplyr::n_distinct(residents$home_code)` FSHCG homes between `r format(date_min, "%e %B %Y")` and `r format(date_max, "%e %B %Y")`. The incidence proportion of COVID-19 symptoms ranged between `r tbrounding(desc_overall$DAT_p_symptomatic*100)`% (based on incidents reports) and `r tbrounding(desc_overall$CT_p_symptomatic*100)`% (based on manager-reported counts). Across the period, care homes registered a total of `r formatbm(desc_overall$CT_total_confirmed)` (`r tbrounding(desc_overall$CT_p_confirmed*100)`% [`r tbrounding((desc_overall$CT_p_confirmed - desc_overall$CT_p_confirmed_ME)*100)`%; `r tbrounding((desc_overall$CT_p_confirmed + desc_overall$CT_p_confirmed_ME)*100)`%]) infections confirmed with nasopharyngeal RNA tests. Incident reports accounted just `r formatbm(desc_overall$unique_residents_test_result_positive)` confirmed infections. In England, `r england_results$n_positive` infections were confirmed of a total `r formatbm(england_results$n_positive + england_results$n_negative)` residents. Comparing with the community, the relative risk of confirmed cases in England was `r tbrounding(RR_england)` [`r tbrounding(exp(log(RR_england) - 1.96 * RR_SE)) `; `r tbrounding(exp(log(RR_england) + 1.96 * RR_SE))`]. The staff-to-bed ratio was the only factor to be independently associated with the rate of confirmed cases: an increase by 10 percentage point in the number of staff relative to the bed total was associated with a 13% reduction [9%, 18%] in the rate of confirmed cases.

**Interpretation** Despite uncertain case ascertainment in care homes, internal surveillance suggests a greater transmission than in the community. Staffing may impact care homes' response to outbreaks and in turn influence rates of confirmed infections The absence of high-quality case ascertainment hinders local infection prevention initiatives which could be better supported by laboratory data surveillance.
...
An important source of epidemiological data on the burden of disease in care homes are outbreak investigations.[@McMichael2020; @Dora2020] One such investigation estimated that 50% of residents and 20% of staff are infected during an outbreak in London, UK.[@Easter6] Estimates of this nature cannot be generalised to the unknown proportion of care homes that have not reported any outbreaks.
...
We used individual-level incident reports and daily tallies to generate estimates of infection incidence in residents and staff. The individual data provided an accurate of the duration of exposure in resident-days as well as right-censoring, since individual records indicate when a resident was discharged or died. A Cox proportional hazards model was used to test the effect of potential risk factors, whether individual- (sex, age, care type) or organisation-level (region, care home size, staff-to-bed ratio). Dates of death were used to estimate the case-fatality rate, as well as compare all-cause mortality based on the presence of symptoms and infection confirmation. A time-dependent Cox proportional hazards model analysed the effect of the same factors on all-cause mortality based on the time-dependent infection classification (susceptible; symptomatic not confirmed; asymptomatic confirmed; symptomatic confirmed).
...
A Datix report may not have been filed for every event. We therefore also estimate the incidence of events using manager counts. The resident-days denominator was computed by subtracting past confirmed cases counts from resident-days estimated from the individual-level data.

The incidence proportion of confirmed cases in England is contrasted with incidence measured from a sample survey of English private households which tested nose/throat swabs from 26,995 individuals between 26 April and 13 June 2020.[@cisbulletin2020; @cisdata2020]
...
arsenal::tableby(home_type  ~ gender + ageg + region_UK +
   user_type2 + admission_type2 + covid_status + status +
   resident_days + susceptible_symptomatic_days + susceptible_confirmed_days, 
...
The overall incidence proportion was between `r tbrounding(desc_overall$DAT_p_confirmed*100)`% [`r tbrounding((desc_overall$DAT_p_confirmed - desc_overall$DAT_p_confirmed_ME)*100)`%; `r tbrounding((desc_overall$DAT_p_confirmed + desc_overall$DAT_p_confirmed_ME)*100)`%] according to incidents reports and  `r tbrounding(desc_overall$CT_p_confirmed*100)`% [`r tbrounding((desc_overall$CT_p_confirmed - desc_overall$CT_p_confirmed_ME)*100)`%; `r tbrounding((desc_overall$CT_p_confirmed + desc_overall$CT_p_confirmed_ME)*100)`%]  according to manager counts. For England only, a total of `r england_results$n_positive` confirmed cases occurred out of a total `r formatbm(england_results$n_positive + england_results$n_negative)` residents. In comparison, the survey of English community households[@cisbulletin2020] found a total of `r cis_data$n_positive` confirmed cases out of `r formatbm(cis_data$n_positive + cis_data$n_negative)` sampled individuals. This implies a relative risk of `r tbrounding(RR_england)` [`r tbrounding(exp(log(RR_england) - 1.96 * RR_SE)) `; `r tbrounding(exp(log(RR_england) + 1.96 * RR_SE))`].




```{r km_confirmed, fig.height=4,fig.width=8, warning=F}
survfit(Surv(time = start_int, 
             time2 = stop_int + 1,
             event = confirmed) ~ region_UK, data = residents_overview) %>% 
  ggsurvplot(., conf.int = TRUE, 
           ...
           legend.labs = sort(unique(residents_overview$region_UK)),
           conf.int.alpha = .1) 
```

`r fig_nums("fig_KM_residents_confirmed")`

```{r KM_CT_residents_infection, fig.height = 4, fig.width = 8}
...
# cph_conf <- coxph(Surv(time = start_int, time2 = stop_int +1, event = confirmed) ~ gender + ageg+home_type+user_type2+region_UK+ staff_to_bed_ratio+bed_tot_category, data = residents_overview)
# summary(cph_conf)
# cox.zph(cph_conf)
...
cph_death2 <- coxph(Surv(date1, date2, death_status) ~
                      ...
                      home_type*covid_status+ cluster(resident_id), data = residents_episodes)
# summary(cph_death)
# cox.zph(cph_death)
# survfit(Surv(time = date1, time2 = date2, event = death_status)~ gender, data = residents_episodes)
# 
explanatory = c("gender", "ageg", "user_type2", "home_type",
                "staff_to_bed_ratio", "bed_tot_category", 
...
survfit(Surv(time = date1, time2 = date2, event = death_status) ~ covid_status + cluster(resident_id), data = residents_episodes) %>% 
  ggsurvplot(., conf.int = TRUE, 
           legend = "right",
           ggtheme = theme_bw(),
           break.x.by	= 7,
...
```{r KM_Staff_Symptomatic, fig.height = 4, fig.width = 8}
...
```{r KM_Staff_Confirmed, fig.height = 4, fig.width = 8}
...
This study was to our knowledge the first to report on the incidence of SARS-CoV-2 infections in UK care homes on a large scale, using individal-level resident records from a large provider network. Findings confirm a very rapid progression of the epidemic across care home residents, with up to `r formatbm(desc_overall$CT_p_symptomatic)`% of residents experiencing symptoms at some stage and `r `r tbrounding(desc_overall$CT_p_confirmed*100)`% [`r tbrounding((desc_overall$CT_p_confirmed - desc_overall$CT_p_confirmed_ME)*100)`%; `r tbrounding((desc_overall$CT_p_confirmed + desc_overall$CT_p_confirmed_ME)*100)`%] having a positive nasopharyngeal antigen test result between `r format(date_min, "%e %B %Y")` and `r format(date_max_tallies, "%e %B %Y")`. Regional variation was considerable, but the cross-examination of from incident reports and manager counts produces inconsistent findings, suggesting strong variation in recording practices. Rates of testing across time and care homes were not yet known at the time of writing and likely distorts measures of incidence.

One prominent organisational factor was significantly and independently associated with the risks of confirmed infection: the staff-to-bed ratio at baseline (***Haydn, please explain***) was a protective factor. A 10 percentage point increase was associated with a 13% reduction [9%; 18%] in hazards of confirmed infections. Few organisational variables were available to identify potential confounders or mediators of this association. One hypothesis is that staffing ratios may be a sign of better-resourced care homes, and of a greater capability to prescribe and observe effective infection control procedures. Furthermore, staff can become a vector in the transmission of infections. It is possible that higher staffing levels mitigate the transmission of infections since each staff member will care for a lower number of residents. Better staffed homes may also be less reliant on bank or agency staff who work in different locations. A more detailed survey of care home characteristics, along with protocols and measures adopted during the epidemic may help elucidate mechanisms through which staffing levels are associated with lower infection rates.

staff infection rates

Similar to infection rates, all-cause mortality exhibited strong regional variation. Nursing home mortality was 76% higher, and . I

Uncertainty remains regarding the internal validity of estimates presented in this report. Beyond uncertain on the exact number of cases, the viral antigen test campaign is a likely subject of under-ascertainment, on account of both the diagnostics' intrinsically low sensitivity, and of the timing of the systematic test campaign initiated on 11 May 2020. Residents may not have provided sample at their peak viral load, thereby reducing the rate of detection. According to a meta-analysis of 7 studies,[@Kucirka2020] the probability of a false-negative result in an infected person on the day of symptom onset (day 0) was 38% [18%; 65%], decreasing to 20% [12%; 30%] on day 3, subsequently rising to 66% [54%; 77%] on day 16. Consequently findings presented in the current study are likely to be underestimates. This includes estimates of the incidence ratio comparing care homes with the community in England.
